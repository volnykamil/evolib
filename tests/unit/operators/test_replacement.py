...
from evolib.core.genotype import IntegerGenotype, BinaryGenotype, RealGenotype
...
def test_population_size_truncation():
	...
	new_pop = strat.replace(parents, offspring, population_size=4)
	assert len(new_pop) == 4
	# When population_size larger than offspring slice length fallback still respects requested size due to slicing logic
